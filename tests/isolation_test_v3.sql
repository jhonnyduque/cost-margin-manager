-- ============================================================================
-- TEST DE AISLAMIENTO (ISOLATION TEST) - V3 (IDEMPOTENTE & ROBUSTO)
-- ============================================================================
-- Instrucciones:
...
-- 0. CLEANUP PREVIO (Para evitar errores de duplicados si el test falló antes)
-- Borramos en orden inverso de dependencias
DELETE FROM product_materials WHERE materialId IN (SELECT id FROM raw_materials WHERE company_id IN ('11111111-1111-1111-1111-111111111111', '22222222-2222-2222-2222-222222222222'));
...
-- Nota: auth.users es tricky borrar si hay claves foráneas, pero users tiene CASCADE delete desde auth.users
-- Intentamos borrar auth.users
...
-- Force authorized role to test RLS
...
-- ============================================================================
ROLLBACK;
-- RESET ROLE; -- No necesario si rollback funciona, pero buena práctica si ejecutas manual
