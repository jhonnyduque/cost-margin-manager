...
export const notificationListener = {
    ...
    start() {
        if (this._channel) {
            console.info('[NotificationListener] Reusing existing channel');
            return this._channel;
        }
        if (this._isSubscribing) {
            console.info('[NotificationListener] Subscription in progress');
            return this._channel as any;
        }

        this._isSubscribing = true;
        console.info('[NotificationListener] Starting listener for event_bus');
        ...
        const channel = supabase
            ...
            .on(
                ...
                async (payload) => {
                    console.info('[NotificationListener] Event received', payload.new?.event_key);
                    await this.handleEvent(payload.new);
                }
            )
            .subscribe((status) => {
                console.info('[NotificationListener] Channel status:', status);
                if (status === 'SUBSCRIBED') {
                    this._channel = channel;
                    this._isSubscribing = false;
                }
                if (status === 'CHANNEL_ERROR' || status === 'CLOSED') {
                    // Reintento mínimo: liberar canal y re-suscribir
                    this._channel = null;
                    this._isSubscribing = false;
    ...
    /**
     * Permite desmontar limpiamente el listener.
     */
    stop() {
        if (this._channel) {
            console.info('[NotificationListener] Unsubscribing channel');
    ...
    async handleEvent(event: any) {
        try {
            ...
            if (user_id) {
                const allowed = await notificationService.shouldNotify(user_id, event_key, 'in_app', { scope: 'user' });
                if (!allowed) {
                    console.log(`[NotificationListener] Evento ${event_key} filtrado por preferencias del usuario ${user_id}`);
                    return;
                }
            } else {
                console.info('[NotificationListener] Evento sin user_id, preferencias no aplican a scope company/global', { event_key, company_id });
            }
            ...
            switch (event_key) {
                ...
                default:
                    console.info('[NotificationListener] Evento ignorado (sin handler)', event_key);
                    break;
            }
        } catch (err) {
