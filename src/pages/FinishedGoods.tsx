...
const FinishedGoods: React.FC = () => {
    ...
    const stockStats = useMemo(() => products.map(product => {
    ...
    /* ── Shared: expanded detail (kardex + FIFO) ── */
    const renderExpandedDetail = (stat: typeof filteredData[0]) => {
        ...
        return (
            <div style={{ padding: 'var(--space-24)', background: 'var(--surface-page)', borderTop: 'var(--border-default)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-24)', flexWrap: 'wrap', gap: 'var(--space-16)' }}>
                    <h4 className="text-small text-muted" style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-8)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.12em' }}>
                        <History size={16} style={{ color: 'var(--text-muted)' }} /> Histórico de Movimientos
                    </h4>
                        ...
                        <div className="tabs" style={{ display: 'inline-flex', overflowX: 'auto' }}>
                            {(['7d', '30d', '90d', 'all'] as const).map(period => (
                                <button key={period} onClick={e => { e.stopPropagation(); setKardexPeriod(period); }} className={kardexPeriod === period ? 'tab is-active' : 'tab'}>
                                    {period === '7d' ? '7d' : period === '30d' ? '30d' : period === '90d' ? '90d' : 'Todo'}
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-12)' }}>
                    {reservedDispatches.map(d => (
                        <div key={d.id} className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-16)', borderLeft: '4px solid var(--state-warning)', flexWrap: 'wrap' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-16)' }}>
                                ...
                                <div>
                                    ...
                                    <p className="text-small text-muted">BORRADOR · {d.client_name || 'Sin Cliente'}</p>
                                </div>
                            </div>
                    ...
                    {movements.map(m => {
                        ...
                        return (
                            <div key={m.id} className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-16)', flexWrap: 'wrap' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-16)' }}>
                ...
                {/* FIFO lots — scroll horizontal en móvil (datos de auditoría) */}
                <div style={{ marginTop: 'var(--space-24)', paddingTop: 'var(--space-24)', borderTop: 'var(--border-default)' }}>
                    <h4 className="text-small text-muted" style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-8)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 'var(--space-16)' }}>
                        <Layers size={14} style={{ color: 'var(--text-muted)' }} /> Lotes FIFO
                    </h4>
                    <div style={{ overflowX: 'auto', borderRadius: 'var(--radius-lg)', border: 'var(--border-default)' }}>
                            ...
                            <tbody>
                                {stat.productionLots.length === 0 ? (
                                    ...
                                    <tr key={lot.id} style={{ opacity: lot.isExhausted ? 0.45 : 1 }}>
                                        ...
                                        <td className="text-small text-muted">{lot.reference || '—'}</td>
                                        ...
                                        <td style={{ textAlign: 'center' }}>
                                            {lot.isExhausted ? <span className="text-small text-muted">Agotado</span> : <span className="text-small text-secondary">Activo</span>}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
        ...
        <PageContainer>
            <SectionBlock>
                ...
                <div className="no-print" style={{ marginTop: 'var(--space-32)', borderTop: 'var(--border-default)', paddingTop: 'var(--space-32)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', gap: 'var(--space-12)', alignItems: 'center' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 'var(--space-12)', alignItems: 'center' }}>
                        ...
                        <Button variant="ghost" size="sm" icon={<FileDown size={18} />} title="Exportar CSV" onClick={exportToCSV} />
                    </div>
                </div>
            </SectionBlock>
                    ...
                    <>
                        ...
                        <div className="cards-mobile">
                            {filteredData.map(stat => {
                                ...
                                return (
                                    <div key={stat.id} style={{ borderRadius: 'var(--radius-xl)', border: 'var(--border-default)', background: 'var(--surface-card)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
                                        {/* Card header — tap to expand */}
                                        <div
                                            onClick={() => setExpandedProductId(isExpanded ? null : stat.id)}
                                            style={{ padding: 'var(--space-16)', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 'var(--space-12)' }}
                                        >
                                            <div className="card-mobile-row-header">
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-12)', minWidth: 0 }}>
                                                    <div style={{ padding: 'var(--space-8)', background: 'var(--surface-muted)', borderRadius: 'var(--radius-lg)', color: 'var(--text-muted)', flexShrink: 0 }}>
                                                        <Package size={18} />
                                                    </div>
                                                    <div style={{ minWidth: 0 }}>
                                                        <p style={{ fontWeight: 800, color: 'var(--text-primary)', textTransform: 'capitalize', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{stat.name}</p>
                                                        ...
                                                        {isLowStock && (
                                                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-4)', fontSize: '10px', fontWeight: 800, color: 'var(--state-warning)', textTransform: 'uppercase' }}>
                                            ...
                                            <div className="card-mobile-row-meta">
                                                <div>
                                                    <p style={{ fontSize: 'var(--text-caption-size)', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)' }}>Físico</p>
                                                    <p style={{ fontWeight: 700, color: 'var(--text-secondary)' }}>{stat.physical}</p>
                                                </div>
                                                <div>
                                                    <p style={{ fontSize: 'var(--text-caption-size)', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)' }}>Disponible</p>
                                                    <p style={{ fontWeight: 700, color: stat.available > 0 ? 'var(--state-success)' : stat.available < 0 ? 'var(--state-danger)' : 'var(--text-muted)' }}>
                                                        {stat.available < 0 && <AlertCircle size={12} style={{ display: 'inline', marginRight: 2 }} />}{stat.available}
                                                    </p>
                                                </div>
                                                <div>
                                                    <p style={{ fontSize: 'var(--text-caption-size)', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)' }}>Valuación</p>
                                                    <p style={{ fontWeight: 800, color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(stat.totalValuation)}</p>
                                                </div>
                                                {stat.reserved > 0 && (
                                                    <div>
                                                        <p style={{ fontSize: 'var(--text-caption-size)', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)' }}>Reservado</p>
                                                        <p style={{ fontWeight: 700, color: 'var(--text-secondary)' }}>{stat.reserved}</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {/* Expanded detail */}
                                        {isExpanded && renderExpandedDetail(stat)}
                                    </div>
                                );
                            })}
                        </div>
                                                ...
                                                <tr style={{ cursor: 'pointer' }} onClick={() => setExpandedProductId(isExpanded ? null : stat.id)}>
                                                    <td>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-12)' }}>
                                                            <div style={{ padding: 'var(--space-8)', background: 'var(--surface-muted)', borderRadius: 'var(--radius-lg)', color: 'var(--text-muted)' }}><Package size={20} /></div>
                                                            <div>
                                                                <p style={{ fontWeight: 800, color: 'var(--text-primary)', textTransform: 'capitalize' }}>{stat.name}</p>
                                                                <p className="text-small text-muted" style={{ fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{stat.reference || 'SIN SKU'}</p>
                                                                {isLowStock && (
                                                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-4)', marginTop: 'var(--space-4)', fontSize: '10px', fontWeight: 800, color: 'var(--state-warning)', textTransform: 'uppercase' }}>
                                                                        <AlertCircle size={10} /> Stock Mínimo
                                                                    </span>
                                                                )}
                                                            </div>
                                                    ...
                                                    <td className="align-right tabular" style={{ fontWeight: 700, color: stat.reserved > 0 ? 'var(--text-secondary)' : 'var(--text-muted)' }}>{stat.reserved > 0 ? stat.reserved : '---'}</td>
                                                    <td className="align-right">
                                                        <span className="tabular" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'flex-end', gap: 'var(--space-4)', fontWeight: 700, color: stat.available > 0 ? 'var(--state-success)' : stat.available < 0 ? 'var(--state-danger)' : 'var(--text-muted)' }}>
                                                            {stat.available < 0 && <AlertCircle size={14} />}{stat.available}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td style={{ textAlign: 'center' }}>
                                                        {isExpanded ? <ChevronUp size={20} style={{ color: 'var(--text-secondary)' }} /> : <ChevronDown size={20} style={{ color: 'var(--text-muted)' }} />}
                                                    </td>
                                                </tr>
