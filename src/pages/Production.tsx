...
const Production: React.FC = () => {
  ...
  const activeProducts = useMemo(
    () => products.filter(p => (p.status || 'activa') === 'activa' && (p.materials || []).length > 0),
    [products]
  );
  ...
  const { activeOrders, completedOrders, metrics } = useMemo(() => {
    const active = productionOrders.filter(o =>
      ['planned', 'preparation', 'processing'].includes(o.status)
    );
    ...
    return {
      ...
      metrics: {
        ...
        finishedToday: completed.filter(
          o =>
            o.status === 'finished' &&
            new Date(o.completed_at || o.created_at).toDateString() === new Date().toDateString()
        ).length,
      },
    };
  }, [productionOrders]);
  ...
  const currentProduct = useMemo(
    () => products.find(p => p.id === productionModal.productId) || null,
  ...
  const handleConfirmBatchProductionWithStatus = async (
    ...
    try {
      await consumeStockBatch(
        product.id,
        productionModal.quantity,
        productionModal.targetPrice,
        status
      );
  ...
  const handleConfirmDebtProduction = async () => {
    try {
      await consumeStockBatch(
        missingStockModal.productId,
        missingStockModal.quantity,
        missingStockModal.targetPrice
      );

      const product = products.find(item => item.id === missingStockModal.productId);
      const totalCost = product
        ? calculateProductCost(product, batches, rawMaterials, unitsOfMeasure) *
        missingStockModal.quantity
        : 0;
      ...
      alert(
        `Error registrando producción con deuda: ${error?.message || 'Error desconocido'}`
      );
    }
  };
  ...
  const renderStatusBadge = (order: ProductionOrder) => {
    switch (order.status) {
      case 'planned':
        return <Badge variant="neutral">🔬 Planeación</Badge>;
      case 'preparation':
    ...
    if (order.status === 'finished' || order.status === 'cancelled') {
      return (
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate(`/productos/detalle/${order.product_id}`)}
        >
          Auditar
        </Button>
      );
      ...
      >
        {order.status === 'planned' && (
          <Button
            variant="secondary"
            size="sm"
            onClick={() => handleTransition(order.id, 'preparation')}
        ...
        {(order.status === 'planned' || order.status === 'preparation') && (
          <Button
            variant="secondary"
            size="sm"
            onClick={() => handleTransition(order.id, 'processing')}
            icon={<Play size={14} />}
          >
            A Procesamiento
          </Button>
        ...
        {(order.status === 'planned' ||
          ...
          order.status === 'processing') && (
            <Button
              variant="primary"
              size="sm"
              onClick={() => handleTransition(order.id, 'finished')}
    ...
    <PageContainer>
      ...
      <div id="print-header" className="print-only" aria-hidden="true">
        <div>
          ...
          >
            {activeProducts.length} productos producibles · {sessionLots} lotes procesados en sesión
          </div>
        </div>
        ...
        >
          ...
          >
            <div style={{ position: 'relative', minWidth: 0 }}>
              ...
              <input
                type="text"
                placeholder="Buscar producto para producción..."
                value={searchTerm}
      ...
      <div id="print-area" className="print-desktop-area" style={{ marginTop: 'var(--space-32)' }}>
        <SectionBlock>
          {productionCards.length === 0 ? (
            <EmptyState
              icon={Factory}
              title="No hay productos listos para producción"
              description="Crea productos con receta o ajusta la búsqueda para comenzar."
            ...
            >
              <table className="table">
                ...
                <tbody>
                  {productionCards.map(
                    ({ product, unitCost, stock, hasCoverage, maxCoveredProduction }) => (
                      <tr key={product.id}>
                        <td>
                          <span
                            style={{
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <div
                            title={
                              hasCoverage
                                ? 'Este lote tiene cobertura completa con el inventario disponible.'
                                : 'Este lote requiere deuda de inventario para completarse.'
                            }
                            style={{
                              display: 'inline-flex',
        ...
        <SectionBlock>
          ...
          >
            {[
              ...
              },
              {
                ...
                value: metrics.preparation,
                color: 'var(--text-muted)',
              },
              {
                label: '⚙️ En Procesamiento',
              ...
              <div key={m.label} className="metric-card">
                <div className="metric-label" style={{ color: m.color }}>
                  {m.label}
                </div>
                <div className="metric-value" style={{ fontSize: 'var(--text-h2-size)' }}>
                  {m.value}
                </div>
              </div>
            ))}
          ...
          >
            <div>
              ...
              >
                Gestiona el flujo desde la planeación hasta el producto final.
              </p>
            </div>
            ...
            >
              <button
                ...
                className={activeTab === 'active' ? 'tab is-active' : 'tab'}
              >
                      ...
                      return (
                        <tr key={order.id}>
                          ...
                          <td>
                            ...
                            <p className="text-small text-muted">
                              {order.unit_cost
                                ? `${formatCurrency(order.unit_cost)} / ud`
                                : 'Pendiente'}
                            </p>
                          </td>
            ...
            {productionModal.isOpen && currentProduct && productionPreview && (
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setProductionModal(c => ({
                          ...c,
                          targetPrice: Math.max(0, Number(e.target.value) || 0),
                        }))
                      }
                    />
                  </div>
                  ...
                  >
                    <div className="inset-card">
                      ...
                      >
                        {formatCurrency(productionPreview.totalCostForBatch)}
                      </p>
                    </div>
                    ...
                    <div className="inset-card">
                      ...
                      >
                        {productionPreview.maxCoveredProduction} lotes
                      </p>
                    </div>
                  </div>
                  ...
                  {productionPreview.missingItems.length > 0 && (
                    <div className="alert alert-warning">
                      <strong>Este lote requiere deuda de inventario.</strong> Se detectaron
                      faltantes en {productionPreview.missingItems.length} insumo
                      {productionPreview.missingItems.length === 1 ? '' : 's'}.
                    </div>
                  )}
                </div>
                ...
                >
                  ...
                  <Button
                    variant="secondary"
                    onClick={() => handleConfirmBatchProductionWithStatus('planned')}
                  >
                    Planificar
                  </Button>
                  <Button
                    variant="primary"
                    onClick={() => handleConfirmBatchProductionWithStatus('finished')}
                    icon={<CheckCircle2 />}
                  >
                    Confirmar ahora
                  </Button>
            ...
            {missingStockModal.isOpen && (
              <Card
                style={{
                  ...
                  padding: 0,
                  overflow: 'hidden',
                  borderColor: 'var(--surface-danger-soft)',
                }}
              >
                <div
                  style={{
                ...
                >
                  ...
                  >
                    No hay stock suficiente para este lote completo. Puedes cancelar o
                    continuar registrando deuda.
                  </p>
                  ...
                  <div className="inset-card">
                    ...
                    >
                      {missingStockModal.maxCoveredProduction} lotes completos
                    </p>
                  </div>
                    ...
                    {missingStockModal.missingItems.map(item => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-small text-muted">
                            Faltan {item.missingQuantity.toFixed(2)} {item.unit}
                          </p>
                        </div>
                        <p
                          style={{
                ...
                >
                  <Button variant="ghost" onClick={closeMissingStockModal}>
                    Cancelar
                  </Button>
                  <Button
                    variant="danger"
                    onClick={handleConfirmDebtProduction}
                    icon={<ArrowRight />}
                  >
            ...
            {successModal && successModal.isOpen && (
              ...
              >
                ...
                >
                  ...
                  >
                    {successModal.quantity} lote{successModal.quantity === 1 ? '' : 's'} de{' '}
                    <strong>{successModal.productName}</strong> se agregaron al inventario.
                  </p>
                  <p
                    style={{
                  ...
                  >
                    Costo estimado: {formatCurrency(successModal.cost)}
                  </p>
                </div>
                ...
                >
                  <Button style={{ width: '100%' }} onClick={() => setSuccessModal(null)}>
                    Entendido
                  </Button>
                </div>
              </Card>
