...
export function CohortHeatmap({ data }: CohortHeatmapProps) {
    ...
    return (
        <div className="h-full w-full rounded-3xl border border-slate-100 bg-white p-6 shadow-sm overflow-hidden">
            <div className="mb-6">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">Cohortes de Retención</h3>
                <p className="text-xs text-slate-500">Porcentaje de retención de ingresos por mes de adquisición.</p>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full border-separate border-spacing-1">
                    <thead>
                        <tr>
                            <td className="p-2 text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50 rounded-lg">Cohorte</td>
                            {Array.from({ length: maxAge + 1 }).map((_, i) => (
                                <td key={i} className="p-2 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50 rounded-lg">
                                    M{i}
                                </td>
                            ))}
                        </tr>
                    ...
                    <tbody>
                        {cohorts.map(month => (
                            <tr key={month}>
                                <td className="p-2 text-[11px] font-bold text-slate-900 bg-slate-50/30 rounded-lg whitespace-nowrap">
                                    {month}
                                </td>
                                {Array.from({ length: maxAge + 1 }).map((_, age) => {
                                    ...
                                    return (
                                        <td key={age} className="p-0">
                                            <div className={`group relative flex h-9 w-full items-center justify-center rounded-lg font-black text-[10px] transition-all hover:scale-105 hover:shadow-lg hover:z-10 cursor-default ${getColor(point.percentage)}`}>
                                                ...
                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 scale-90 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all pointer-events-none z-10">
                                                    <div className="bg-slate-900 text-[10px] text-white p-2 rounded-lg shadow-xl text-center">
                                                        Cohorte: {month}<br />
            ...
            <div className="mt-6 flex items-center justify-between pt-4 border-t border-slate-50">
                <div className="flex items-center gap-1.5">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Menor</span>
                    ...
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Mayor</span>
                </div>
                <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Meta NRR: 110%</span>
            </div>
        </div>
    );
}
